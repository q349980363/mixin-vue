    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          <li class="nav-item">
            <div class="nav-link" @click="del">删除好友</div>
          </li>
          <li class="nav-item">
...
export default {
  ...
  created() {
    ...
    this.store.e.on("friends", (item) => {
      if (item.UserName == this.target || item.Target == this.target) {
  ...
  methods: {
    loadData(callBack) {
      store.http.get("/api/get_friendschat/" + this.target).then((response) => {
    ...
    empty() {
      store.http
        .post("/api/del_friendschat/" + this.target)
        .then((response) => {
          this.loadData();
        });
    },
    del() {
      store.http.post("/api/del_friends/" + this.target).then((response) => {
        this.$router.go(-1);
      });
    },
  },
